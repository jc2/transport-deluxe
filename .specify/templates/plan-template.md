...
- [ ] No docstrings anywhere in the sub-project (Principle VI)
- [ ] Sub-project has a `README.md` with service description and one entry per module (Principle VII)
...
# [REMOVE IF UNUSED] Option 4: Monorepo sub-project (transport-deluxe default)
{project-name}/
├── pyproject.toml
├── src/
│   ├── main.py
│   ├── tools/               # db.py, logging.py, and other cross-cutting utilities
│   └── modules/
│       └── {domain}/
│           ├── router.py    # Endpoint declaration only
│           ├── models.py    # Request/response/DB models
│           └── service.py   # Business logic
└── tests/                   # Integration tests only (external/black-box, _tests DB)
```
