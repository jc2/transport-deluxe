...
async def test_create_config_invalid_range(client: AsyncClient, clean_table, auth_token: str):
    ...
    data = response.json()
    assert "min_days cannot be greater than max_days" in data["messages"][0]
...
async def test_create_config_duplicate(client: AsyncClient, clean_table, auth_token: str):
    ...
    # Second request
    response2 = await client.post("/lead-time-configs", json=payload, headers={"Authorization": f"Bearer {auth_token}"})
