...
import src.modules.fuel_cost_config.mcp_tools  # noqa: F401
from src.modules.fuel_cost_config.admin import FuelCostConfigAdmin
...
from src.modules.fuel_cost_config.router import router
...
app = FastAPI(title="Fuel Cost Configuration Service", lifespan=combine_lifespans(lifespan, mcp_app.lifespan))

admin_auth = AdminAuth(secret_key=os.environ["SESSION_SECRET"])
admin = Admin(app, engine, authentication_backend=admin_auth)
admin.add_view(FuelCostConfigAdmin)
...
app.include_router(router, prefix="/fuel-cost-configs")
app.mount("/", mcp_app)
