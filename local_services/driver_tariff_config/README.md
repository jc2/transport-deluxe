# Driver Tariff Configuration Service
...
This service is responsible for the **first phase of cost calculation** (`all_in_cost`). When the estimation engine needs to compute the cost of a shipment, it calls `/driver-tariff-configs/resolve` to obtain the `fuel_cost_per_km` applicable to that specific load. The returned value is multiplied by the route distance to produce the **Base Cost Adjustment**.
...
|--------|----------------------|
| **[Driver Tariff Configuration](../business/configurations/driver_tariff_configuration.md)** | Core entity. Each record stores the `fuel_cost_per_km` for a Customer + Truck Type combination. |
...
|--------|------|----------------|
| Router | `src/modules/driver_tariff_config/router.py` | Endpoint declarations, parameter validation, delegates to service |
| Models | `src/modules/driver_tariff_config/models.py` | SQLModel DB table, Pydantic request/response models, TruckType enum |
| Service | `src/modules/driver_tariff_config/service.py` | Business logic: list, get, create, update, deactivate, resolve |
...
You can view and manage Driver Tariff Configurations via the SQLAdmin UI.
