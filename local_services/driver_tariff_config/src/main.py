...
import src.modules.driver_tariff_config.mcp_tools  # noqa: F401
from src.modules.driver_tariff_config.admin import DriverTariffConfigAdmin
...
from src.modules.driver_tariff_config.router import router
...
app = FastAPI(title="Driver Tariff Configuration Service", lifespan=combine_lifespans(lifespan, mcp_app.lifespan))

admin_auth = AdminAuth(secret_key=os.environ["SESSION_SECRET"])
admin = Admin(app, engine, authentication_backend=admin_auth)
admin.add_view(DriverTariffConfigAdmin)
...
app.include_router(router, prefix="/driver-tariff-configs")
app.mount("/", mcp_app)
